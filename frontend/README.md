...
顶栏右侧有切换按钮，点击即在浅色与深色之间切换，偏好写入 `localStorage['cc_theme']`；
**首次访问跟随系统**（`prefers-color-scheme`），用户显式切换过之后就固定为所选模式。

- **实现方式**：全部靠 CSS 变量。`src/styles/main.css` 里有 `:root`（浅色令牌）与 `html.dark`（深色令牌）两套；
  `<html>` 上加 `dark` 类即完成切换。Element Plus 自带的深色变量（`element-plus/theme-chalk/dark/css-vars.css`）
  也是挂在 `html.dark` 下的，所以组件库会自动跟随；`main.css` 在其后引入，同权重下覆盖掉 EP 默认的深色主色。
- **首帧防闪白**：`index.html` 里有一段内联脚本，在 Vue 挂载**之前**就读取偏好并打好 `html.dark`
  （否则会先渲染一帧浅色再跳深色）。`src/stores/theme.ts` 的 `init()` 与它共用同一个键名。
- **echarts 需要单独处理**：图表的配色写在 option 里（JS 值），不走 CSS，不会自动跟随。
  `DashboardView` 用 `getComputedStyle` 读当前主题的变量值，并在 `theme.mode` 变化时重新 `setOption`。
- **文件图标**（`FileIcon.vue`）的类型配色是**刻意的语义色**，两套主题共用未改。

> 给后续维护者的提醒：组件里**不要再写死颜色**，一律用 `main.css` 的令牌
> （`--surface` / `--surface-hover` / `--warn-*` / `--success-*` / `--text-*` / `--border*` 等）。
> `--surface` 用于「浮在页面上的面」（原来写 `#fff` 的背景）；彩色底上的白字保持 `#fff` 不动。
...
- **AI 问答**（独立全屏对话模块）：课程选择器（**首次进入默认选中第一门课**）+ 沉浸式对话 + 来源引用追溯
  ...
  - **输入与发送分离**：Agent 输出期间**输入框始终可打字**（可以先把下一个问题写好草稿），
    只禁用「发送」（Enter 或按钮）并给出提示；未选课程时同样允许打字、只在发送时拦。
    ⚠️ 发送守卫必须放在**清空输入框之前**：否则按 Enter 会先清空、再被 composable 因 `streaming` 静默 return，
    结果是草稿被吃掉且消息没发出去
  - **输入框那个按钮随状态切换**：运行中变成「停止接收」（深色圆 + 白色方块），空闲时是发送。
    停止后按钮变回发送，**草稿保留** —— 所以「边输出边打字 → 停止 → 发送」是一条顺畅的路径。
    文案刻意写「停止接收」而非「停止生成」：后端暂无取消接口，这一下只断开前端接收，worker 仍在执行，
    之后可点「继续接收」接回来（后端补上 cancel 接口后，这里只需改文案与调用）
